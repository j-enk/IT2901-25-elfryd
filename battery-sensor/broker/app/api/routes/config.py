...
from core.models import ConfigDataResponse, ConfigCommandResponse
...
@router.get(
    "/config", response_model=List[ConfigDataResponse], summary="Get configuration data"
)
def get_config_data(
    limit: int = Query(
    ...
    try:
        conn = get_connection()
        results = query_config_data(conn, limit, hours, time_offset)
        conn.close()
        return results
...
@router.post(
    "/config/send",
    response_model=ConfigCommandResponse,
    summary="Send configuration command",
)
