...
def query_messages(
    ...
    try:
        ...
        query = sql.SQL("SELECT id, topic, message, timestamp FROM {} WHERE 1=1").format(sql.Identifier(table))
...
def query_sensor_data(
    ...
    try:
        ...
        if hours is not None:
            ...
            query = sql.SQL("{} AND device_timestamp >= %s AND device_timestamp <= %s").format(query)
...
def query_config_data(
    ...
    try:
        ...
        query = sql.SQL("SELECT id, command, topic, timestamp FROM {} WHERE 1=1").format(
            sql.Identifier(table_name)
        )
        params = []
        ...
        if hours is not None:
            if time_offset is not None:
                ...
                query = sql.SQL("{} AND timestamp >= %s AND timestamp <= %s").format(query)
                params.extend([start_time, end_time])
            else:
