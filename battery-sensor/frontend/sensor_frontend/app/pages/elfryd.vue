  ...
  <div class="flex flex-row gap-6 p-4 max-w-[1600px] mx-auto bg-pink-100">
    ...
    <div class="w-2/3 rounded-lg shadow-md p-6">
      ...
      <div class="bg-green-100 rounded-lg p-4 mb-6 flex gap-4 items-end">
        <div class="flex-1">
          ...
          <input id="battery_id" v-model="battery_id" type="text"
            placeholder="Enter topic name or leave blank for all topics" class="w-full p-2 border rounded" />
        </div>

        <div class="flex-1">
          <label for="limit" class="font-semibold block mb-1">Limit:</label>
          <input id="limit" v-model="limit" type="number" min="1" max="100" class="w-full p-2 border rounded" />
        </div>

        <button @click="fetchBatteryData" :disabled="isLoading"
      ...
      <div v-else-if="data" class="mt-4">
        ...
        <pre class="text-red-500 bg-gray-100 p-4 rounded-lg overflow-auto max-h-[500px] font-mono text-sm">
    ...
    <div class="w-1/3 bg-white rounded-lg shadow-md p-6">
      ...
      <div class="mb-6">
        ...
        <div class="flex space-x-2 mb-4">
          <UButton @click="fetchConfig(false)" :loading="configLoading" :disabled="configLoading" class="flex-1"
            color="primary">
            {{ configLoading ? 'Loading...' : 'Fetch Config' }}
          </UButton>

          <UButton @click="fetchConfig(true)" :loading="configLoading" :disabled="configLoading" class="flex-1"
            color="primary">
            Send All Config
          </UButton>
        </div>
        ...
        <div v-if="configData" class="mt-4">
          <div class="bg-orange-50 border border-gray-200 rounded-lg p-4">
            <pre
              class="text-red-500 text-sm font-mono overflow-auto max-h-[300px]">{{ JSON.stringify(configData, null, 2) }}</pre>
          </div>
        </div>
      </div>
    </div>
