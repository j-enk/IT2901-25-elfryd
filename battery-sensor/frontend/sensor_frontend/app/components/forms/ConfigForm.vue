...
async function handleUpdateConfig() {
    if (!configSensorVal.value) {
        sendError.value = 'Please select a sensor'
        return
    }
    ...
    <section class="mb-6 flex flex-col w-full gap-4">
        <label class="form-control w-full">
            ...
            <select v-model="configSensorVal" class="select select-success select-bordered w-full">
                <option v-for="option in configSensor" :key="option.value" :value="option.value">
                    {{ option.label }}
                </option>
            </select>
        </label>
        ...
        <label class="form-control w-full">
            ...
            <div class="w-full">
                <select v-model="configFreqVal" class="select select-success select-bordered w-full pr-10">
                    ...
                    <option v-for="option in configFreq" :key="option.value" :value="option.value">
                        {{ option.label }}
                    </option>
                </select>
            </div>
        ...
        <div v-if="commandResult" class="bg-base-100 border-1 border-neutral rounded-lg p-4 mt-4">
