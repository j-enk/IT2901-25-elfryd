 ...
 */
 ...
 {
     int err;
     char message[512]; /* Increased buffer size for safety */
     size_t offset = 0;
     bool first = true;
     
     /* Format multiple readings with pipe separator */
     for (int i = 0; i < count && offset < sizeof(message) - 32; i++) { /* Leave extra space for safety */
         size_t len;
         char timestamp_str[24]; /* Dedicated buffer for timestamp */
         
         /* Manual conversion of timestamp to string without using %lld */
         int64_t timestamp = readings[i].timestamp;
         char *ts_ptr = timestamp_str + sizeof(timestamp_str) - 1;
         *ts_ptr = '\0'; /* Null terminate */
         
         /* Convert timestamp digit by digit */
         do {
             *(--ts_ptr) = '0' + (timestamp % 10);
             timestamp /= 10;
         } while (timestamp > 0 && ts_ptr > timestamp_str);
         
         /* Move the timestamp string to the beginning of the buffer */
         if (ts_ptr > timestamp_str) {
             memmove(timestamp_str, ts_ptr, strlen(ts_ptr) + 1);
         }
         ...
         /* Debug output to verify timestamp conversion */
         printk("Battery %d timestamp: %lld formatted as: %s\n", 
             readings[i].battery_id, 
             (long long)readings[i].timestamp, 
             timestamp_str);
         
         /* Format: "{battery_id}/{voltage}/{timestamp}" */
         len = snprintf(message + offset, sizeof(message) - offset,
                      "%d/%d/%s", /* Use pre-formatted timestamp string */
                      readings[i].battery_id,
                      readings[i].voltage,
                      timestamp_str);  /* Use the pre-formatted string */
                      
         if (len >= sizeof(message) - offset - 16) { /* Extra margin */
