...
struct bt_gatt_read_params *read_params[MAX_CONNECTIONS];
...
static int cmd_read_uuid(struct bt_conn *conn, int target, uint8_t index)
{
	int err;
	...
	if(param_index >= MAX_CONNECTIONS) {
		printk("Max read params reached\n");
		return -ENOMEM;
	}
	struct bt_gatt_read_params *read_param = read_params[param_index++];

	read_param->func = read_func;
	read_param->handle_count = 0;
	read_param->by_uuid.start_handle = BT_ATT_FIRST_ATTRIBUTE_HANDLE;
	read_param->by_uuid.end_handle = BT_ATT_LAST_ATTRIBUTE_HANDLE;
	uuid.val = target;
	read_param->by_uuid.uuid = &uuid.uuid;
...
static void foreach_function(struct bt_conn *conn, void *data)
{
	...
	int read_err = cmd_read_uuid(conn, BT_UUID_GATT_V_VAL, index);
	if (read_err) {
		printk("  ❌ GATT read failed with err %d\n", read_err);
	} else {
		printk("  📬 GATT read pending\n");
